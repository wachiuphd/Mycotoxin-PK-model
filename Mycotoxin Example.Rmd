...
```{r calibrate using simulated data}
...
for (i in 1:length(parmnames)) {
  iparmnames <- paste0(parmnames[i],1:8,".")
  tmp.samps <- samps[,names(samps) %in% iparmnames]
  tmp.M <- samps[,rep(paste0("M_",parmnames[i]),8)]
  tmp.SD <- samps[,rep(paste0("SD_",parmnames[i]),8)]
  tmp.samps <- exp(tmp.samps*tmp.SD + tmp.M)
  tmp.samps$iter <- 1:nrow(tmp.samps)
  tmp.df <- pivot_longer(tmp.samps,1:8)
